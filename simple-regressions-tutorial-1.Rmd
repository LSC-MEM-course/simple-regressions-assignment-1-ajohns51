...
```{r}
x_01 <- as.numeric(scale(rnorm(n_obs2)))
x_02 <- as.numeric(scale(rnorm(n_obs2)))
x_03 <- as.numeric(scale(rnorm(n_obs2)))

Practice1_df <- data.frame(x_01, x_02, x_03)
```
    
    c. Generate the response data in the same data frame, using the linear model equation.
```{r}
Practice1_df$y <- beta_00 + beta_01 * x_01 + beta_02 * x_02 + beta_03 * x_03 + rnorm(n_obs2, mean = 0, sd = sigma2)
```
    
    d. Fit a model using `lm()`, and pull out the coefficients from the fitted model, to compare with your "ground truth" values.
```{r}
Practice1_model <- lm(y ~ x_01 + x_02 + x_03, data = Practice1_df)
Practice1_model$coefficients
```
"ground true" values were:
Intercept: 5
x_01: 10
x_02: 25
x_03: 100
...
```{r}
Simulate_ParamterEstimates <-
  function(beta0, beta1, beta2, beta3, n_obs = 1000, sigma = 1) {
    df <- data.frame(x_01 <- as.numeric(scale(rnorm(n_obs))),
                     x_02 <- as.numeric(scale(rnorm(n_obs))),
                     x_03 <- as.numeric(scale(rnorm(n_obs))))
    df$y <-
      beta0 + beta1 * x_01 + beta2 * x_02 + beta3 * x_03 + rnorm(n_obs, mean = 0, sd = sigma)
    model <- lm(y ~ x_01 + x_02 + x_03, data = df)
...
```{r}
Simulate_ParamterEstimates(0, 1, 2, 3, n_obs = 1000000)
Simulate_ParamterEstimates(0, 1, 2, 3, n_obs = 5)
Simulate_ParamterEstimates(0, 1, 2, 3, n_obs = 1000, sigma = 100)
Simulate_ParamterEstimates(0, 1, 2, 3, n_obs = 1000000, sigma = 100)
```
...
I am interested in the effects of coarticulation in children with CIs: how does vowel context (front vowels vs. back vowels) affect centroid frequency of children's productions of "t" and "k" consonants, and is the effect of vowel context different across groups (children with CIs, children with normal hearing [NH])?
...
1. Predictor variables for me would be relatively easy: Vowel context (Front, Back) and Target Consonant (t, k) are both binary predictors, determined by the words elicited during the experiment. Group (CI, NH) is another binary predictor, determined by participants' hearing status.
...
2a.To generate parameter values, I would look at adult data to look at the mean and range of centroid frequencies for these consonants. I would choose a similar (but slightly higher) mean centroid value for /t/ front vowel contexts (highest centroid), and then a mean centroid value for /k/ in back vowel contexts (lowest centroid). Then I would choose values for /t/ in back vowel contexts and /k/ in front vowel contexts, which would fall in between the other two values. To simulate data for kids with NH, I would choose a relatively low sigma, because their centroid frequencies are probably more reliable, and for kids with CIs, I would chose a higher sigma, because their productions are generally more variable.
...
sim_results_Long <- sim_results %>% 
  pivot_longer(cols = c(beta_0_hat, beta_1_hat, sigma_hat),
               names_to = "Parameter",
               values_to = "Estimates")
```
...
```{r}
ggplot(sim_results_Long, aes(Estimates)) + geom_histogram()
...
```{r}
ggplot(sim_results_Long, aes(Estimates)) + geom_histogram(binwidth = 1) +
  facet_wrap(~ Parameter, scales = "free")

ggplot(sim_results_Long, aes(Estimates)) + geom_histogram(binwidth = 0.05) +
  facet_wrap(~ Parameter, scales = "free")

ggplot(sim_results_Long, aes(Estimates)) + geom_histogram(binwidth = 0.01) +
  facet_wrap(~ Parameter, scales = "free")
```
    
    d. Play around with colors and/or themes.
```{r}
ggplot(sim_results_Long, aes(Estimates)) + geom_histogram(aes(fill = Parameter), binwidth = .1) +
  facet_wrap( ~ Parameter, scales = "free") +
  scale_fill_manual(values = c("#762a83", "#e7d4e8", "#1b7837")) +
  theme_bw()
```
...
```{r}
SimulateParameters_Plot <-
  function(n_obs, n_sims, beta0, beta1, sigma) {
    ...
    for (this_sim in 1:n_sims) {
      if (this_sim %% 1000 == 0) {
        cat("starting simulation", this_sim, "\n")
      }
      this_fit <- sim_simple_lm(n_obs, beta0, beta1, sigma)
      sim_results[this_sim, 2:4] <- c(this_fit$beta_0_hat,
                                      this_fit$beta_1_hat,
                                      this_fit$sigma_hat)
    }
    ...
    sim_results_Long <- sim_results %>%
      pivot_longer(
        cols = c(beta_0_hat, beta_1_hat, sigma_hat),
        names_to = "Parameter",
        values_to = "Estimates"
      )
    print(
...
SimulateParameters_Plot(100, 1000, 0, 1, 1)
```
...
```{r}
SimulateParameters_Plot(100, 1000, 0, 1, 1)
SimulateParameters_Plot(10000, 5000, 0, 1, 1)
SimulateParameters_Plot(50, 5000, 0, 1, 1)
```
...
```{r}
mysleep$group.c <- ifelse(mysleep$group == "pre", -1, 1)
```
...
```{r}
summary(lm(extra ~ group.c, data = mysleep))
summary(lm(extra ~ group, data = mysleep))
```
The residuals are exactly the same; all the coefficients are different
...
The new intercept corresponds to the overall average of "extra" (mean of extra collapsed across conditions)
```{r}
mean(mysleep$extra)
```
...
```{r}
AcousticData <- read.csv("EH_AcousticData_Updated10.8.csv")
summary(AcousticData)
```
...
```{r}
summary(lm(Centroid.kHz ~ TargetC, data = AcousticData))
AcousticData %>% group_by(TargetC) %>% 
  summarise(mean=mean(Centroid.kHz))
```
...
```{r}
AcousticData$TargetC.c <- ifelse(AcousticData$TargetC == "k", -1, 1)
summary(lm(Centroid.kHz ~ TargetC.c, data = AcousticData))
mean(AcousticData$Centroid.kHz)
```
...
```{r}
model <- lm(data = irisLocal, formula = Sepal.Width ~ Species)
summary(model)
```
...
```{r}
contrasts(irisLocal$Species)
irisLocal <-
  irisLocal %>% mutate(
    veriscolor.dummy = ifelse(Species %in% "versicolor", 1, 0),
    virginica.dummy = ifelse(Species %in% "virginica", 1, 0)
  )
model2 <-
  lm(data = irisLocal,
     formula = Sepal.Width ~ veriscolor.dummy + virginica.dummy)
summary(model2)
summary(model)
```
...
```{r}
irisLocal <-
  irisLocal %>% mutate(Species2 = relevel(Species, ref = "versicolor"))
contrasts(irisLocal$Species2)
```


5. Re-fit the model from #2 using the new factor from #4.  Explain the differences between coefficients.
```{r}
model3 <- lm(data = irisLocal, formula = Sepal.Width ~ Species2)
summary(model3)
```
Now, the intercept represents the mean value of extra for the versicolor reference category, the first parameter estimate is the mean change in extra from versicolor to setosa, and the second parameter estimate is the mean change in extra from versicolor to verginica
...
irisLocal <-
  irisLocal %>% mutate(
    setosa.dummy = ifelse(Species %in% "setosa", 1, 0))

model4 <-
  lm(data = irisLocal,
     formula = Sepal.Width ~ setosa.dummy + virginica.dummy)
summary(model4)
summary(model3)
```
Nailed it.
